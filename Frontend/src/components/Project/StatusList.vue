    ...
    <v-form @submit.prevent>
        <v-container fluid>
            ...
            <v-row class="px-4 bg-primary mt-10">
                <v-col>Title</v-col>
                <v-col cols="1" class="d-flex align-center ga-1">
                    <span>System</span>
                    <v-tooltip location="top">
                        <template v-slot:activator="{ props }">
                            <v-icon size="x-small" class="cursor-auto" icon="mdi-help-circle-outline" v-bind="props">
                            </v-icon>
                        </template>
                        Generated by system
                    </v-tooltip>
            ...
            <template v-for="(item, index) in statusList" :key="index">
                <v-row class="border border-t-0 ps-4" :class="[{ 'bg-white': !dark }]">
                    ...
                    <v-col cols="1">{{ item.isSystem ? 'Yes' : 'No' }}</v-col>
